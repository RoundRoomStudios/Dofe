#ifndef IRON_BASIC

#define IRON_BASIC

#include <stdlib.h>
#include <stdio.h>

#define DEBUG true
#define DEBUG_PCF false
#define DEBUG_PCF_TDIR false
#define DEBUG_PCF_METR false
#define DEBUG_PCF_ENCO false
#define DEBUG_PCF_BITM false
#define DEBUG_PCF_BITM_MAIN false
#define DEBUG_PCF_BITM_GLYF false
#define DEBUG_PCF_BITM_INFO false
#define DEBUG_TXT false

static inline int8_t max8(int8_t a, int8_t b) {
    return a > b ? a : b;
}

static inline int8_t min8(int8_t a, int8_t b) {
    return a < b ? a : b;
}

static inline int16_t max16(int16_t a, int16_t b) {
    return a > b ? a : b;
}

static inline int16_t min16(int16_t a, int16_t b) {
    return a < b ? a : b;
}

static inline int32_t max32(int32_t a, int32_t b) {
    return a > b ? a : b;
}

static inline int32_t min32(int32_t a, int32_t b) {
    return a < b ? a : b;
}

static inline int64_t max64(int64_t a, int64_t b) {
    return a > b ? a : b;
}

static inline int64_t min64(int64_t a, int64_t b) {
    return a < b ? a : b;
}

static inline uint8_t maxU8(uint8_t a, uint8_t b) {
    return a > b ? a : b;
}

static inline uint8_t minU8(uint8_t a, uint8_t b) {
    return a < b ? a : b;
}

static inline uint16_t maxU16(uint16_t a, uint16_t b) {
    return a > b ? a : b;
}

static inline uint16_t minU16(uint16_t a, uint16_t b) {
    return a < b ? a : b;
}

static inline uint32_t maxU32(uint32_t a, uint32_t b) {
    return a > b ? a : b;
}

static inline uint32_t minU32(uint32_t a, uint32_t b) {
    return a < b ? a : b;
}

static inline uint64_t maxU64(uint64_t a, uint64_t b) {
    return a > b ? a : b;
}

static inline uint64_t minU64(uint64_t a, uint64_t b) {
    return a < b ? a : b;
}

static inline void* allocate(size_t size, const char* errorType, const char* dataName) {
    // Helper malloc with error checking
    void* x = malloc(size);
    if (x == NULL) {
        printf("%s | Failed to malloc %s.\n", errorType, dataName);
        return NULL;
    }
    return x;
}

static const uint8_t bitSwapTable[0x100] = {
    
};

static inline uint8_t bitSwap8(uint8_t byte) {
    return bitSwapTable[byte];
}

static inline uint16_t bitSwap16(uint16_t shortVal) {
    uint16_t newVal = bitSwap8(shortVal&0xFF);
    newVal |= bitSwap8((shortVal>>8)&0xFF)<<8;
    return newVal;
}

static inline uint32_t bitSwap32(uint32_t intVal) {
    uint32_t newVal = bitSwap8(intVal&0xFF);
    newVal |= bitSwap8((intVal>>8)&0xFF)<<8;
    newVal |= bitSwap8((intVal>>16)&0xFF)<<16;
    newVal |= bitSwap8((intVal>>24)&0xFF)<<24;
    return newVal;
}

typedef struct BitWriter {
    uint8_t* bytes;
    uint64_t byte;
    uint64_t bitMask;
    uint8_t bitMaskSize;
} BitWriter;

static inline uint64_t getBitPos(BitWriter* w) {
    return w->byte * 8 + w->bitMaskSize;
}

static inline void pushBits(BitWriter* w, uint32_t bits, uint8_t amount) {
    // add to bitmask
    w->bitMask <<= amount;
    w->bitMaskSize += amount;
    w->bitMask |= bits;

    // add to bytes
    while (w->bitMaskSize >= 8) {
        w->bitMaskSize -= 8;
        w->bytes[w->byte] = (uint8_t)(w->bitMask >> w->bitMaskSize);
        w->byte++;
    }
}

static inline void pushBufferBits(BitWriter* w) {
    w->bytes[w->byte] = (uint8_t)(w->bitMask << 8 - w->bitMaskSize);
    w->bitMaskSize = 0;
    w->bitMask     = 0;
}

typedef struct BitReader {
    uint8_t* bytes;
    uint64_t bit;
} BitReader;

static inline bool readBits(BitReader* r, uint64_t* buffer, int8_t amount) {
    while (amount > 0) {
        uint8_t readAmount = minU8(amount, 8 - (r->bit % 8));
        uint8_t mask = ((1 << readAmount) - 1) << (8 - (r->bit % 8) - readAmount);

        *buffer <<= readAmount;
        *buffer |= (r->bytes[r->bit / 8] & mask) >> (8 - (r->bit % 8) - readAmount);

        amount -= readAmount;
        r->bit += readAmount;
    }

    return true;
}

#endif
